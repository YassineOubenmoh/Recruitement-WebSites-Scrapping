...
public class nvEtudeChartsRekrute {
    public static void main(String[] args) {
        ...
        try {
            ...
            // Modify the query to get count for each 'niveau_etude' with occurrences more than 5
            String query = "SELECT niveau_etude, COUNT(*) AS count FROM rekrute GROUP BY niveau_etude HAVING count > 5";
            ...
            while (resultSet.next()) {
                String niveau_etude = resultSet.getString("niveau_etude");

                // Vérifier si la clé n'est pas nulle avant de l'ajouter au dataset
                if (niveau_etude != null) {
                    int count = resultSet.getInt("count");
                    dataset.setValue(count, "Offres", niveau_etude);
                    System.out.println(niveau_etude + " : " + count + " offres");
                }
            }

            if (dataset.getRowCount() > 0) {
                JFreeChart chart = ChartFactory.createBarChart(
                        "Répartition des offres par niveau d'étude pour le site Rekrute", // Titre du graphique
